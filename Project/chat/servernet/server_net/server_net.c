...
#include "list.h"							/* doubly link list */
...
#define SEND_FUNC_LEFT(Func, Data, Context) \
{											\
	if ( Func != NULL )						\
	{										\
		Func(Data, Context) ;				\
...
static void CloseClientSocket(Server *_server, int _clientSocket, void* _context) ;
...
static int ReciveAndSentData(void* _clientSocket, void* _server)
{
	...
	{  
		...
		{
			CloseClientSocket( server, clientSocket, server->m_context  ) ;     
			return 0 ;			
		}
		else
		{
			FUNC_NEW_MSG(server)( server->m_context, clientSocket, buffer, sizeof(buffer) );      
		}
		ACTIVITY(server)-- ;    
	} 
...
static int InitRestArgsServer(Server *_server, ServerArgs *_serverArgs) 
{
	...
	_server->m_context = _serverArgs->m_context;
	return OK ;
}
...
static void CloseClientSocket(Server *_server, int _clientSocket, void* _context) 
{
	SEND_FUNC_LEFT( CLIENT_LEFT(_server), _clientSocket, _context ) ;
