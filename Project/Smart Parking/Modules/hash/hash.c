...
{	
	...
	if(isFound)
	{				
		if(isFound  != ListItrEnd(_map->m_listArrMap[bucketIndex]))
		{
			*_pValue = ((Pair*)ListItrGet(isFound))->m_item;
			return MAP_SUCCESS;
		}
	}
...
static size_t ForEachBucket(const HashMap* _map, KeyValueActionFunction _action, void* _context)
{
	...
	for(i=0;i<_map->m_capacity;++i)
	{
		begin = ListItrBegin(_map->m_listArrMap[i]);
		end = ListItrEnd(_map->m_listArrMap[i]);
		if(begin)
		{			
...
static void DestroyAllBuckets(HashMap*_map, void (*_keyDestroy)(void* _key), void (*_valDestroy)(void* _value))
{
	...
	for(i=0;i<(_map)->m_capacity;++i)
	{			
		start = ListItrBegin((_map)->m_listArrMap[i]);
		end = ListItrEnd(((_map)->m_listArrMap[i]));
...
{	
	...
	bucketIndex = GET_IND(_map,_key);	
	begin = ListItrBegin(_map->m_listArrMap[bucketIndex]);
	end = ListItrEnd(_map->m_listArrMap[bucketIndex]);	
...
static void HashCopyBucket(List** _dest, List** _src, size_t _oldCap,size_t _newCap, HashFunction _hashFunc, int _ind)
{
	...
	bucketBegin = ListItrBegin(_src[_ind]);
	bucketEnd = ListItrEnd(_src[_ind]);

	while(!ListItr_Equals(bucketBegin,bucketEnd))
	{
		key = HashGetKey((Pair*)ListItrGet(bucketBegin));
		index = GetIndex(_newCap,_hashFunc,key);
		if(!_dest[index])
		{
			_dest[index] = ListItrCut(bucketBegin,ListItrNext(bucketBegin));
			bucketBegin = ListItrBegin(_src[_ind]);
		}			
		else
		{
			ListItrSplice(((ListItr)_dest[index]),bucketBegin,ListItrNext(bucketBegin));
			bucketBegin = ListItrNext(bucketBegin);
		}				
	}
