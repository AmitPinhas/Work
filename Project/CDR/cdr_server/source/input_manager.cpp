...
template <typename Type>  
void InputManager<Type>::Run(DataStorage& a_refDS, size_t a_numThreads,  int a_portNum, int a_sizeListenQueue, int a_maxClients)
{
    ...
    DequeueThreads(a_refDS, a_numThreads);  

    threadrunServer.Join();
}

template <typename Type>  
void InputManager<Type>::DequeueThreads(DataStorage& a_refDS ,size_t a_numThreads)
{
    for(size_t index = 0; index < a_numThreads; ++index)
    {
        DequeueCdr<Type> *dequeue = new DequeueCdr<Type>( m_cdrs, a_refDS);
...
template <typename Type>
DequeueCdr<Type>::DequeueCdr(WaitableQueue<Type>& a_cdrs, DataStorage& a_refDS)
: m_cdrs(a_cdrs)
, m_refDS(a_refDS)
{
}
