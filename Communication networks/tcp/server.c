...
{          
       ...
       while (  ! ListItrEquals( res , endClientSockets ) )
       {                      
                res = ListItrForEach( beginClientSockets, endClientSockets , ReciveData, _serverArgs ); 
                
                if ( ! ListItrEquals( res , endClientSockets ) )
                {
...
static int CheckNewClient ( ServerArgs *_serverArgs)
{
	...
	while ( res != -1 )
	{
	       ...
	       if ( res > 0 )
	       {
	        ...
	        if ( _serverArgs -> m_clientCount >= MAX_CLIENTS-1 )
	        {
	                close ( res ) ;
	                return FUNC_SUCCEEDED ;
	        }  
                 ...
                 if ( (res2 = SocketNonBlock( res ) ) != FUNC_SUCCEEDED )
                 {
                      return res2 ;
                 }	
