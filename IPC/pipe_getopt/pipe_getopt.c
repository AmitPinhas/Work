...
#define MILI_SEC(T) (1000*(T))

int main (int argc , char **argv)
{
	int option ,flagVerbose=0 ,numberOfWrite=0 ;
	int writerWorkTime=0 , readerWork=0 , sizeOfReadReq=0 ,  countWrite=0 , countRead=0 , i;
>>>>>>> 52904f7c48d4791a5042f99af8c4e055d2ff6ebd
...
int InItArgs( Args *_args, int _argc, char **_argv )
{
	...
	while ((option = getopt( _argc, _argv, "vn:w:r:s:" )) != -1)
=======
			
	while ((option = getopt(argc , argv , "vn:w:r:s:")) != -1)
>>>>>>> 52904f7c48d4791a5042f99af8c4e055d2ff6ebd
	{
		switch (option)
		{
...
=======
	...
	buf = (char*)malloc( ( sizeOfReadReq + 1));
	if ( NULL == buf )
>>>>>>> 52904f7c48d4791a5042f99af8c4e055d2ff6ebd
	{
		++(*_countWrite);
...
=======
	...
	{ 
		...
		for ( i = 0 ; i < numberOfWrite ; i++ )
		{
				++countWrite;
				
				if ( flagVerbose )
				{
					printf("Child Process Writer #%d\n" , countWrite);
				}
				
				usleep(MILI_SEC(writerWorkTime));
				sprintf(strWrite, "Message %2d\n", countWrite);  
				
				write(WRITE_PIPE , strWrite , strlen(strWrite));  
		}
		close(WRITE_PIPE);					
	}
	else
	{
		...
		close(WRITE_PIPE);			
		while ( (nBytes = read(READ_PIPE, buf, sizeOfReadReq)) > 0 )
		{
			++countRead;
			buf[nBytes] = 0;
			if ( flagVerbose )
	...
	if ( readerWork > writerWorkTime )
	{
		if ( pid != 0 )
		{
			printf("Writer #%s of writter\n", buf);
			printf("Reader #%d of reads\n", countRead);
		}		
	}
	else
	{
